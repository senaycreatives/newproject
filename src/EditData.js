...
export function EditData() {
  ...
  useEffect(() => {
    if (fetchedData && fetchedData?.data) {
      ...
      const fetchedKeys = fetchedData?.data
        ? Object.keys(fetchedData?.data)
        : [];
            ...
            return {
              ...
              type:
                valueType === "boolean"
                  ...
                  ? "number"
                  : valueType === "object" &&
                    fetchedData?.data[key] instanceof Date
                  ? "date"
                  : "string",
              required: false,
            };
  ...
  const mutation = useMutation({
    mutationFn: (data) => {
      return axios.put(
        "https://server.industrialclearance.co.uk/updatedata",
        data,
        {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://server.industrialclearance.co.uk/getsingledata/${zetacode}`,
          {
            headers: { Authorization: authHeader() },
          }
        );
        ...
        <div className="w-full relative h-screen flex items-center justify-center overflow-hidden">
          ...
          <div className="flex sm:relative  sm:overflow-y-hidden flex-col  rounded-md  sm:h-[97%]  mb-[10px] overflow-x-hidden w-[100%]">
            ...
            <div className="flex w-[90%] sm:h-[80%] relative pt-[30px] items-center justify-center">
              <form
                className="w-full h-full flex-wrap  flex flex-col"
